...
const emits = defineEmits<{
  (event: 'closeGameButton'): void;
}>()

const closePLayButton = () => {
  emits('closeGameButton')
}
...
<!--      </button>-->
 ...
 <div class="flex justify-center">
   <div class="dark:bg-lightgray bg-gray-100 rounded-full h-[44px] w-[44px] flex justify-center items-center">
     <img src="@/assets/images/game.png" alt="game-image"/>
   </div>
 </div>
  <div class="space-y-2">
    <div>
      <p class="dark:text-white text-extra-small">Time for yet another fun session</p>
    </div>
    <div class="w-full">
      <button @click="goToPlayPage" class="btn btn-sm btn-ghost rounded-lg hover:scale-105 transition-all duration-300 ease-in-out w-full bg-casablanca-300">
        <span class="text-extra-small">Play</span>
      </button>
    </div>
  </div>
